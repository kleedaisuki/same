...
- **Safety**: ignore negation semantics are unchanged. The ignore policy excludes `.same` by name; Windows performs an additional identity query only for root-level case variants such as `.SAME`, preserving the distinction on case-sensitive volumes without probing ordinary directories. Reparse/symlink entries are not descended, and metadata comes from `FileReader` rather than enumeration timestamps. As before, this is not protection against malicious concurrent ancestor replacement or a filesystem snapshot.
