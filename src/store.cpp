...
void exec(sqlite3* db, const char* sql) {
    if (sqlite3_exec(db, sql, nullptr, nullptr, nullptr) != SQLITE_OK) fail(db, "SQLite execution");
}
class Statement {
public:
    Statement(sqlite3* db, const char* sql) : db_(db) {
        if (sqlite3_prepare_v2(db, sql, -1, &stmt_, nullptr) != SQLITE_OK) fail(db, "SQLite prepare");
    }
    ...
    void string(int index, std::string_view value) { bytes(index, value.data(), value.size()); }
    void integer(int index, sqlite3_int64 value) {
        if (sqlite3_bind_int64(stmt_, index, value) != SQLITE_OK) fail(db_, "SQLite bind integer");
    }
    bool step() {
    ...
    sqlite3_int64 integer(int index) const { return sqlite3_column_int64(stmt_, index); }
    ...
    FileRecord record() const {
        ...
        for (const unsigned char byte : size) result.stamp.size = (result.stamp.size << 8) | byte;
...
Store::Store(const std::filesystem::path& path, std::size_t cache_bytes) : impl_(std::make_unique<Impl>()) {
    ...
    try {
        exec(db, "CREATE TABLE IF NOT EXISTS files("
                 ...
                 "digest BLOB NOT NULL CHECK(length(digest)=32),generation INTEGER NOT NULL) WITHOUT ROWID;"
                 "CREATE INDEX IF NOT EXISTS files_content ON files(size,digest,path);"
                 "CREATE TABLE IF NOT EXISTS scan_state(id INTEGER PRIMARY KEY CHECK(id=1),generation INTEGER NOT NULL);"
...
Store::~Store() { rollback_scan(); }
void Store::begin_scan() {
    if (impl_->scanning) throw std::logic_error("Scan already active");
    exec(impl_->db, "BEGIN IMMEDIATE");
    impl_->scanning = true;
    try {
        Statement query(impl_->db, "SELECT generation FROM scan_state WHERE id=1");
        if (!query.step()) throw std::runtime_error("Missing state generation");
...
std::optional<FileRecord> Store::cached(std::string_view path) {
    Statement query(impl_->db, "SELECT path,size,identity,modified,changed,digest FROM files WHERE path=?1");
    query.string(1, path);
    if (!query.step()) return std::nullopt;
    return query.record();
}
void Store::save(const FileRecord& record) {
    if (!impl_->scanning) throw std::logic_error("No active scan");
    Statement insert(impl_->db, "INSERT INTO files(path,size,identity,modified,changed,digest,generation)"
...
void Store::visit_candidates(const std::function<void(const FileRecord&)>& visitor) {
    if (impl_->scanning) throw std::logic_error("Commit scan before comparing");
    Statement query(impl_->db, "SELECT f.path,f.size,f.identity,f.modified,f.changed,f.digest FROM files f "
    ...
    while (query.step()) visitor(query.record());
}
void Store::clear_representatives() { exec(impl_->db, "DELETE FROM representatives"); }
...
void Store::visit_representatives(const std::function<bool(const FileRecord&)>& visitor) {
    Statement query(impl_->db, "SELECT path,size,identity,modified,changed,digest FROM representatives ORDER BY path");
    while (query.step()) if (!visitor(query.record())) break;
}
void Store::reset_matches() { exec(impl_->db, "DELETE FROM matches"); }
...
void Store::visit_matches(const std::function<void(std::string_view, std::string_view)>& visitor) {
    Statement query(impl_->db, "SELECT m.representative,m.member FROM matches m JOIN "
