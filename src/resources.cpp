...
Resources::Resources(const Config& config) : capacity_(config.queue_capacity) {
    ...
    for (std::size_t i = 0; i < config.workers; ++i) {
        ...
        if (config.backend != "cpu")
            worker->compute = try_cuda_compute(config.block_bytes, config.device_memory_bytes / config.workers);
        if (worker->compute) ++gpu_workers_;
        else worker->compute = make_cpu_compute();
        workers_.push_back(std::move(worker));
    }
    try {
        for (auto& worker : workers_) threads_.emplace_back([this, ptr = worker.get()] { run(*ptr); });
    } catch (...) { close(); throw; }
}
...
void Resources::enqueue(std::function<void(Worker&)> task) {
    ...
    if (closed_) throw std::runtime_error("resource manager is closed");
